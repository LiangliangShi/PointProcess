# -*- coding: utf-8 -*-
"""
Created on Fri May 17 16:11:51 2019
...
class BPPPMD(object):
    def __init__(self, n_seq,d,regular=False,penalty=0.2):
        ...
        with tf.name_scope('train'):
            ...
            if self.regular==True:
                loss+=tf.contrib.layers.l1_regularizer(penalty)(self.alpha)
...
if __name__ == '__main__':
    ...
    alpha = np.array([0.5,0,0.3,0,-0.2,0.6,0.2,0,0.2]).reshape([3,3])
    beta = 1
    T = 2000
    ...
    n = len(seqs)
    d = len(mu)
    model = BPPPMD(n,d,regular=True,penalty=0.2)
    ...
    for i in range(100000):
        ...
        if i%1000==1:
            print('logli:',sess.run(model.logliklihood,feed_dict={model.ex:ex1, model.Gx:Gx1, model.mux:mux1}))
